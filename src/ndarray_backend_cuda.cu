...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if(gid < size){
    ...
    for(int dim = shape.size - 1; dim >= 0; dim--){
      int idx_in_dim = current % shape.data[dim]; // Compute index for the current dimension
      in_idx += idx_in_dim * strides.data[dim];   // Use strides to compute position in non-compact array
      current /= shape.data[dim];                 // Move to the next dimension
    }
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if(gid < size){
    ...
    for(int dim = shape.size - 1; dim >= 0; dim--){
      int idx_in_dim = current % shape.data[dim]; // Compute index for the current dimension
      out_idx += idx_in_dim * strides.data[dim];   // Use strides to compute position in non-compact array
      current /= shape.data[dim];                 // Move to the next dimension
    }
...
__global__ void MatmulKernel(const scalar_t* A, const scalar_t* B, scalar_t* C,
  ...
  for (uint32_t tile_idx = 0; tile_idx < (N + TILE - 1) / TILE; ++tile_idx) {
    // Load the A and B tiles into shared memory, taking care of bounds
    if (row < M && (tile_idx * TILE + threadIdx.x) < N) {
      tile_A[threadIdx.y][threadIdx.x] = A[row * N + tile_idx * TILE + threadIdx.x];
    } else {
      tile_A[threadIdx.y][threadIdx.x] = 0.0;
    }

    if (col < P && (tile_idx * TILE + threadIdx.y) < N) {
      tile_B[threadIdx.y][threadIdx.x] = B[(tile_idx * TILE + threadIdx.y) * P + col];
    } else {
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size,size_t reduce_size) {
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size){
    scalar_t current_max = a[gid * reduce_size];
    for(int p= 0;p < reduce_size;p++){
       current_max= max(current_max,a[gid*reduce_size+p]);
    }
    out[gid] = current_max;
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t size,size_t reduce_size) {
  ...
  if (gid < size){
    ...
    for(int p= 0;p < reduce_size;p++){
       current_sum += a[gid*reduce_size+p];
    }
    out[gid] = current_sum;
