...
class RNNCell(Module):
    ...
    def forward(self, X, h=None):
        ...
        if self.bias:
            return self.nonlinearity(X @ self.W_ih + self.bias_ih.reshape((1, self.hidden_size)).broadcast_to((batch_size, self.hidden_size)) + h @ self.W_hh + self.bias_hh.reshape((1, self.hidden_size)).broadcast_to((batch_size, self.hidden_size)))
        else:
...
class RNN(Module):
    ...
    def forward(self, X, h0=None):
        ...
        for idx, cell in enumerate(self.rnn_cells):
            ...
            h_n.append(h)
        return ops.stack(current_inputs, 0), ops.stack(h_n, 0)
...
class LSTMCell(Module):
    ...
    def forward(self, X, h=None):
        ...
        if h is None:
          h0, c0 = init.zeros(batch_size, self.hidden_size, device=self.device, dtype=self.dtype), init.zeros(batch_size, self.hidden_size, device=self.device, dtype=self.dtype)
        else:
          h0,c0 = h
        
        if self.bias:
          all_gates = X @ self.W_ih + self.bias_ih.reshape((1, 4 * self.hidden_size)).broadcast_to((batch_size, 4 * self.hidden_size)) \
        ...
        gates = []
        for i in range(4):
            gates.append(ops.stack(all_gates_split[i * self.hidden_size : (i + 1) * self.hidden_size], axis = 1))
        i,f,g,o = gates
...
class Embedding(Module):
    def __init__(self, num_embeddings, embedding_dim, device=None, dtype="float32"):
        ...
        # Initialize weights from N(0, 1) as specified
        self.weight = Parameter(
            init.randn(num_embeddings, embedding_dim, device=device,mean = 0.0,std = 1.0, dtype=dtype, requires_grad=True)
        )
        ### END YOUR SOLUTION
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Create one-hot vectors using the indices
        # This creates a tensor of shape (seq_len, bs, num_embeddings)
        one_hot = init.one_hot(self.num_embeddings, x, device=x.device, dtype=x.dtype).reshape((seq_len * batch_size, self.num_embeddings))
        #one_hot = init.one_hot(self.num_embeddings, x, device=x.device, dtype=x.dtype)
        # Multiply with weights to get embeddings
        # Shape: (seq_len, bs, num_embeddings) @ (num_embeddings, embedding_dim)
        # Result shape: (seq_len, bs, embedding_dim)
        return (one_hot @ self.weight).reshape((seq_len, batch_size, self.embedding_dim))
        ### END YOUR SOLUTION