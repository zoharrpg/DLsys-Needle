...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if not self.is_compact():
          raise ValueError("Reshape is only supported on compact arrays")
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        new_shape = tuple([self._shape[x] for x in new_axes])
    ...
    def broadcast_to(self, new_shape):
        ...
        assert len(new_shape) >= len(self._shape), "New shape must have at least as many dimensions as current shape."

        for i in range(1, len(self._shape) + 1):
          if self._shape[-i] != 1:
            assert new_shape[-i] == self._shape[-i], f"Dimension {i} in new shape must match existing shape or be 1 for broadcasting."
          
        new_strides = [0] * (len(new_shape) - len(self._shape)) + list(self._strides)
        for i in range(1, len(self._shape) + 1):
          if self._shape[-i] == 1:
            new_strides[-i] = 0  # Broadcast this dimension by making the stride 0
    ...
    def __getitem__(self, idxs):
        ...
        for sl,stride in zip(idxs,self._strides):
          ...
          # Update the offset to reflect the new starting point
          new_offset += sl.start * stride;

          # Calculate the new shape and stride for each dimension
          new_shape.append((sl.stop - sl.start -1 ) // sl.step+1)
          
          new_strides.append(stride * sl.step)
    ...
    def flip(self, axes):
        ...
        # Compute new strides and offset to achieve flipping
        new_strides = list(self._strides)
        new_offset = self._offset

        # For each axis that needs flipping, we reverse the stride and adjust the offset
        for axis in axes:
          new_strides[axis] = -new_strides[axis]
        # Adjust offset to the end of the axis, effectively reversing it
          new_offset += (self.shape[axis] - 1) * self._strides[axis]

        # Create a new view with modified strides and offset
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        new_shape = tuple(self._shape[i] + axes[i][0] + axes[i][1] for i in range(self.ndim))

        # Create a new NDArray with the computed shape, filled with zeros.
        padded_array = NDArray.make(new_shape, device=self.device)
        padded_array.fill(0)

        # Calculate the slices for placing the original array within the padded array
        original_slices = tuple(slice(axes[i][0], axes[i][0] + self._shape[i]) for i in range(self.ndim))

        # Set the central region of the padded array to the original array values
        padded_array[original_slices] = self
